...
// صفحه اصلی پرزرق‌وبرق و انتخاب بخش‌ها
                ...
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      gradient: const LinearGradient(colors: [Colors.pinkAccent, Colors.purpleAccent, Colors.blueAccent]),
                      boxShadow: [
                        BoxShadow(
  ...
  Widget _buildFancyMenuCard(BuildContext context, {required String title, required String subtitle, required IconData icon, required List<Color> gradientColors, required VoidCallback onTap}) {
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(colors: gradientColors, begin: Alignment.topLeft, bottomRight: Alignment.bottomRight),
          borderRadius: BorderRadius.circular(20),
          boxShadow: [
            BoxShadow(
...
// بخش موزیک با قابلیت جستجو، پلیر صوتی و دانلود مستقیم
...
class _MusicScreenState extends State<MusicScreen> {
  ...
  final List<Map<String, dynamic>> _chatItems = []; // کانتینر پیام‌های موزیک شبیه تلگرام
  ...
  Future<void> _searchAndSendMusic(String query) async {
    ...
    try {
      // استفاده از یوتیوب‌اکسپلود برای یافتن ترک صوتی بر اساس جستجوی متنی کاربر
      ...
      if (searchResults.isNotEmpty) {
        ...
        String duration = video.duration != null ? "${video.duration!.inMinutes}:${(video.duration!.inSeconds % 60).toString().padLeft(2, '0')}" : "03:30";
      ...
      setState(() {
        _chatItems.add({
          "type": "system",
          "text": widget.currentLanguage == 'fa' ? 'خطا در جستجوی موزیک: $e' : 'Error searching music: $e',
        });
      });
    } finally {
      setState(() {
  ...
  Future<void> _downloadMusic(MusicMessageModel music) async {
    ...
    if (!status.isGranted && !await Permission.manageExternalStorage.isGranted) {
      // حتی اگر دسترسی رده‌بالا بخواهد، از پوشه اپلیکیشن استفاده می‌کنیم تا حتما ذخیره شود
    }
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          content: Text(widget.currentLanguage == 'fa' ? 'موزیک با موفقیت در پوشه Download ذخیره شد!' : 'Music successfully saved in Downloads!'),
        ),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          if (item["type"] == "user") {
                            ...
                            // حباب موزیک شبیه تلگرام با پلیر صوتی مدرن
...
// صفحه چت هوش مصنوعی (حفظ ساختار کامل قبلی)
